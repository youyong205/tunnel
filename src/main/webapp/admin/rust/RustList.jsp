    ...
    <div class="row-fluid">
      ...
      <div class="span10"> 
     		<%@include file="./../MeasuringHeader.jsp"%>
      		<div class="row-fluid">
      			<div class="span12">
      				<form class="text-right form-inline margin-buttom" action="rustList.do" method="post">
						<strong>选择隧道</strong>
      		...
      		<form action="rustBatchDelete.do" id="form" method="post" onsubmit="return checkForm();">
				...
				<table class="table table-striped table-bordered table-condensed table-hover">
				 <thead><tr>
					...
					<th width="27%">检测时间</th>
					<th width="20%">形状</th>
					<th width="20%">锈蚀面积(m㎡)</th>
					<th width="20%">
					<t:privilege res="衬砌环锈蚀检测模块:新增">
						<a class="btn btn-small btn-info" href="rustAdd.do?tunnelId=<s:property value="tunnelId"/>&tunnelSectionId=<s:property value="parentTunnelSectionId"/>&liningRingConstructionId=<s:property value="liningRingConstructionId"/>&index=<s:property value="index"/>" >新增</a>
						<a class="btn btn-small btn-info" href="rustBatchAdd.do?tunnelId=<s:property value="tunnelId"/>&tunnelSectionId=<s:property value="parentTunnelSectionId"/>&liningRingConstructionId=<s:property value="liningRingConstructionId"/>&index=<s:property value="index"/>" >导入</a>
					</t:privilege>
					<t:privilege res="衬砌环锈蚀检测模块:删除">
						<button  class="btn btn-small btn-danger" type="submit">批量删除</button></td>
					</t:privilege>
					</th>
				</tr></thead><tbody>
				<s:iterator value="rusts" status="vs">
					<tr>
					...
					<td>
					<t:privilege res="衬砌环锈蚀检测模块:详情">
						<a class="btn btn-small btn-success" href="rustDetail.do?tunnelId=<s:property value="tunnelId"/>&tunnelSectionId=<s:property value="parentTunnelSectionId"/>&rustId=<s:property value="id"/>&liningRingConstructionId=<s:property value="liningRingConstructionId"/>&index=<s:property value="index"/>">详情</a>
					</t:privilege>
					<t:privilege res="衬砌环锈蚀检测模块:编辑">
						<a class="btn btn-small btn-primary" href="rustUpdate.do?tunnelId=<s:property value="tunnelId"/>&tunnelSectionId=<s:property value="parentTunnelSectionId"/>&rustId=<s:property value="id"/>&liningRingConstructionId=<s:property value="liningRingConstructionId"/>&index=<s:property value="index"/>">编辑</a>
					</t:privilege>
					<t:privilege res="衬砌环锈蚀检测模块:删除">
						<a class="delete btn  btn-small btn-danger" href="rustDelete.do?tunnelId=<s:property value="tunnelId"/>&tunnelSectionId=<s:property value="parentTunnelSectionId"/>&rustId=<s:property value="id"/>&liningRingConstructionId=<s:property value="liningRingConstructionId"/>&index=<s:property value="index"/>">删除</a>
					</t:privilege>
					</td>
					</tr>
				</s:iterator></tbody>
