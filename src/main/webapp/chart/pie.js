    ...
    Venus.SvgChart.addChart('pie', {
        draw: function(opt) {
            ...
            */
            ...
            var series = this.series.getSeries().slice(0),
                colors = this.colors,
                paper = this.stage,
                data,
                total = 0,
            ...
            /**
            *bind sector elements event actions   Բ�Ķ�����Ӧ����
            */
            var allState = {};
            function bindElementsAction() {
                elements && elements.forEach(function(item, i) {
                    allState[opts[i].index] = 0;
                    item.click(function() {
                        status = allState[opts[i].index];
                        if (status == 1) {
                            this.transform('s1,1,' + this.cx + ',' + this.cy);
                            allState[opts[i].index] = 0;
                        } else {
                            this.transform('s1.1,1.1,' + this.cx + ',' + this.cy);
                            allState[opts[i].index] = 1;
                        }
                    })
