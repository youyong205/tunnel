...
public class CuringServiceImpl implements CuringService {
	...
	public int queryCuringSizeByType(int tunnelId, int tunnelSectionId, int componentId, String type) {
		try {
			return m_curingDao.queryCuringSizeByType(tunnelId, tunnelSectionId,componentId, type);
		} catch (Exception e) {
			m_logger.error(e.getMessage(), e);
			return -1;
		}
	...
	public List<Curing> queryLimitedCuringsByType(int tunnelId, int tunnelSectionId, int componentId,String type, int start, int size) {
		try {
			return m_curingDao.queryLimitedCuringsByType(tunnelId, tunnelSectionId,componentId, type, start, size);
		} catch (Exception e) {
			m_logger.error(e.getMessage(), e);
