...
public class BracketAction extends ScheduledAction {
	...
	public String getActionModule() {
		return Constrants.s_bracket_model;
	}
	...
	public String bracketAddSubmit() {
		try {
			...
			if (id > 0) {
				Log log = createLog(Constrants.s_bracket_model, Constrants.s_operation_add, m_bracket);
	...
	public String bracketDelete() {
		try {
			...
			if (count > 0) {
				Log log = createLog(Constrants.s_bracket_model, Constrants.s_operation_delete,
				      m_bracketId);
	...
	public String queryAllBrackets() {
		m_brackets = m_bracketService.queryLimitedBrackets(m_tunnelId,
	...
	public String bracketUpdate() {
		try {
			...
			m_tunnelSections = m_tunnelSectionService.queryLimitedTunnelSectionsByTunnelId(
			      m_bracket.getTunnelId(), 0, Integer.MAX_VALUE);
	...
	public String bracketUpdateSubmit() {
		try {
			...
			if (count > 0) {
				Log log = createLog(Constrants.s_bracket_model, Constrants.s_operation_update,
				      m_bracket);
