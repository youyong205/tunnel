...
public class BuriedSectionAction extends FileUploadAction {
	...
	public String buriedSectionList() {
		try {
			if (m_tunnelId == 0) {
				m_tunnels = m_tunnelService.queryAllTunnels();
				m_totalSize = m_buriedSectionService.queryAllSize();
				m_totalPages = computeTotalPages(m_totalSize);
				int start = (m_index - 1) * SIZE;
				if (start < 0) {
					start = 0;
				}
				m_buriedSections = m_buriedSectionService.queryLimitedBuriedSections(start, SIZE);
			} else {
				m_tunnels = m_tunnelService.queryAllTunnels();
